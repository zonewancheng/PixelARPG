...
function drawDrops(ctx, drops, animate = true) {
    ...
    const glowSize = animate ? 36 + Math.sin(Date.now() / 150) * 6 : 36; // 放大2倍
    
    // 皮肤掉落放最后渲染（层级最高）
    const sortedDrops = [...drops].sort((a, b) => {
        if (a.item?.type === 'skin') return 1;
        if (b.item?.type === 'skin') return -1;
        return 0;
    });
    
    sortedDrops.forEach(d => {
        if (!d.item) return;
        const drawY = d.y + (animate ? Math.sin(time + d.x * 0.1) * 4 : 0); // 放大2倍
        ...
        if (d.item.type === 'treasure' && d.item.id === 'gold') {
            ...
            if (window.renderConsumableIcon) {
                const iconUrl = window.renderConsumableIcon(d.item, 48); // 放大2倍
                if (iconUrl) {
                    const img = new Image();
                    img.src = iconUrl;
                    ctx.drawImage(img, d.x - 24, drawY + 4, 48, 48); // 放大2倍
                }
            } else {
                ctx.font = '48px Arial'; // 放大2倍
                ctx.textAlign = 'center';
                ctx.fillText(d.item.icon || '💰', d.x, drawY + 32); // 放大2倍
            }
            // 显示金币数量
            ctx.font = 'bold 22px Arial'; // 放大2倍
            ctx.textAlign = 'center';
            ctx.fillStyle = '#ffd700';
            ctx.strokeStyle = '#000';
            ctx.lineWidth = 4; // 放大2倍
            const goldValue = d.item.value || 10;
            ctx.strokeText(goldValue, d.x, drawY - 12); // 放大2倍
        ...
        if (window.renderEquipmentIcon && ['weapon', 'armor', 'helmet', 'boots', 'ring', 'necklace'].includes(d.item.type)) {
            const itemCanvas = window.renderEquipmentIcon(d.item, 40); // 放大2倍
            ctx.drawImage(itemCanvas, d.x - 20, drawY + 8, 40, 40); // 放大2倍
            // 显示装备名称 - 根据品质显示颜色
            ctx.font = 'bold 20px Arial'; // 放大2倍
            ctx.textAlign = 'center';
            // 品质颜色
            const qualityColors = {
                'common': '#fff',
            ...
            if (skin) {
                ...
                if (window.SpriteManager && typeof window.SpriteManager.drawCharacterFrame === 'function') {
                    // 创建临时canvas使用SpriteManager渲染
                    const tempCanvas = document.createElement('canvas');
                    tempCanvas.width = previewSize;
                    tempCanvas.height = previewSize;
                    const tempCtx = tempCanvas.getContext('2d');
                    
                    window.SpriteManager.drawCharacterFrame(tempCtx, skin, 'down', 'idle', 0, previewSize);
                    ctx.drawImage(tempCanvas, previewX, previewY, previewSize, previewSize);
                } else {
                    // 回退：简化版渲染
                    ...
                    ctx.fill();
                    ctx.fillStyle = skin.clothesColor || '#4a9eff';
                    ...
                    ctx.fillStyle = skin.skinColor || '#ffe4d0';
                    ctx.beginPath();
...
function drawPlayerAttack(ctx, player) {
    ...
    // 放大2倍：攻击范围160像素，匹配实际判定范围
    const attackRadius = 160;
    const attackArc = Math.PI * 2 / 3; // 120度扇形（匹配dot > -0.5）
    ...
    const crescentWidth = 50; // 放大2倍（原25）
    
    // 月牙外发光
    ctx.shadowColor = slashColor;
    ctx.shadowBlur = 40; // 放大2倍（原20）
