...
#### enemies.js - 敌人数据
```javascript
window.ENEMY_TYPES        // 4种普通怪物
window.BOSS_TYPES         // 7种Boss
window.createEnemyFromType() // 创建敌人实例
```
...
### 7.1 添加新物品
在 `js/data/items.js` 的 `BASE_ITEMS` 数组中添加。

### 7.2 添加新技能
在 `js/data/skills.js` 的 `SKILLS` 数组中添加。

### 7.3 添加新怪物
在 `js/data/enemies.js` 的 `ENEMY_TYPES` 或 `BOSS_TYPES` 中添加。
