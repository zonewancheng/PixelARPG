# Pixel Hero ARPG - 项目说明文档

## 1. 项目概述

### 1.1 游戏简介
Pixel Hero ARPG 是一款基于 HTML5 Canvas 的实时动作角色扮演游戏（ARPG）。玩家在像素风格的游戏世界中冒险，击败怪物、获取装备，提升等级，挑战强大的 Boss。

### 1.2 技术栈
- **前端**: 纯 HTML5 + CSS3 + JavaScript（ES6）
- **渲染**: HTML5 Canvas 2D Context
- **无需依赖**: 不使用任何第三方库，纯原生实现

### 1.3 核心特性
- 实时战斗系统（ARPG）
- 装备系统（武器、服装、帽子、靴子、戒指、项链）
- 技能系统（6种投射技能）
- 物品掉落与自动拾取
- **Boss战斗（每关卡随机Boss，拥有固定技能）**
- 等级成长系统
- **装备品质系统**（白/绿/蓝/紫/金）
- **商店系统**（购买/出售/刷新）
- **无限背包**（无容量限制）
- **角色面板**（装备外观显示）
- **地图系统**（墙壁、草地、水域、树木、花朵、小山）
- **天气系统**（白云、乌云、闪电）
- **呼吸动画**（玩家、敌人）
- **IndexedDB存档**
- **图鉴系统**（怪物/技能/装备，自动追踪发现）
- **伤害数字系统**（玩家/怪物受伤、HP/MP回复）
- **雷电云系统**（对玩家和怪物造成闪电伤害）
- **确认弹窗**（出售装备时弹出确认）
- **模块化架构**（数据模块 + UI模块）
- **统一渲染系统**（所有UI使用与游戏一致的像素渲染）
- **精美像素装备**（武器/护甲/头盔/靴子/戒指/项链，各有独特设计）
- **精美像素技能**（6种技能，图标与效果一致）
- **精美像素怪物与Boss**（8种怪物 + 7种Boss，独特造型与动画）

---

## 2. 文件结构

```
PixelARPG/
├── index.html              # 游戏入口文件
├── css/
│   ├── style.css         # 基础样式
│   ├── panels.css        # 统一面板样式
│   ├── character.css     # 角色面板样式
│   ├── shop.css         # 商店面板样式
│   ├── inventory.css    # 背包面板样式
│   └── bestiary.css    # 图鉴面板样式
├── js/
│   ├── data/            # 数据模块
│   │   ├── qualities.js # 装备品质数据
│   │   ├── items.js     # 装备/物品数据 + 渲染函数
│   │   ├── skills.js    # 技能数据 + 渲染函数
│   │   ├── enemies.js   # 怪物/Boss数据 + 渲染函数
│   │   └── world.js    # 地图/天气数据
│   ├── ui/              # UI模块
│   │   ├── panelManager.js # 面板管理器
│   │   ├── compare.js   # 对比面板
│   │   ├── inventory.js # 背包面板
│   │   ├── character.js # 角色面板
│   │   ├── shop.js     # 商店面板
│   │   └── bestiary.js # 图鉴面板
│   ├── renderUtils.js  # 统一渲染工具
│   ├── renderer.js      # 渲染函数
│   └── game.js         # 主游戏逻辑
├── tests/
│   ├── data-tests.html  # 数据模块测试
│   └── ui-tests.html   # UI模块测试
└── README.md            # 项目说明文档
```

---

## 3. 模块说明

### 3.1 数据模块 (js/data/)

#### qualities.js - 装备品质
```javascript
window.ITEM_QUALITIES      // 5种品质定义
window.EQUIPMENT_SLOTS     // 6种装备槽
window.QUALITY_ORDER       // 品质优先级
```

#### items.js - 物品数据 + 渲染
```javascript
window.BASE_ITEMS          // 基础物品定义
window.createItem()         // 创建物品实例
window.generateRandomItem() // 生成随机物品
window.getItemStats()     // 获取物品属性描述
window.renderPlayerSprite()// 渲染玩家外观
window.renderPlayerIcon()  // 渲染玩家头像
window.renderEquipmentIcon()// 渲染装备图标
```

#### skills.js - 技能数据 + 渲染
```javascript
window.SKILLS              // 6种技能定义
window.getSkillById()     // 根据ID获取技能
window.renderSkillIcon()   // 渲染技能图标
```

#### enemies.js - 敌人数据 + 渲染
```javascript
window.ENEMY_TYPES         // 8种普通怪物
window.BOSS_TYPES          // 7种Boss
window.discoverEnemy()    // 记录击杀
window.renderEnemyIcon()  // 渲染怪物图标
```

#### world.js - 世界环境
```javascript
window.TILE_TYPES          // 7种地图瓦片
window.TILE_COLORS         // 瓦片颜色
window.TREE_TYPES          // 5种树木
window.HILL_TYPES          // 3种小山
window.FLOWER_COLORS       // 花朵颜色
window.CLOUD_TYPES         // 3种云
window.initClouds()        // 初始化云朵
window.getTileColor()     // 获取瓦片颜色
```

### 3.2 UI模块 (js/ui/)

#### panelManager.js - 面板管理器
```javascript
window.PanelManager
  .isAnyOpen()            // 检查是否有面板打开
  .isPanelOpen(name)      // 检查指定面板是否打开
  .closeAll()             // 关闭所有面板
  .openPanel(name)        // 打开指定面板
  .closePanel(name)       // 关闭指定面板
  .togglePanel(name)      // 切换面板状态
```

#### compare.js - 对比面板
```javascript
window.UICompare
  .show(item, slot, source) // 显示对比面板
  .hide()                  // 隐藏对比面板
  .equipItem()            // 装备物品
```

#### inventory.js - 背包
```javascript
window.UIInventory
  .init()                 // 初始化
  .open()                 // 打开背包
  .close()                // 关闭背包
  .render()               // 渲染内容
  .equipItem(item)        // 装备物品
  .quickSell(quality)     // 一键出售
```

#### character.js - 角色
```javascript
window.UICharacter
  .init()                 // 初始化
  .open()                 // 打开角色面板
  .close()                // 关闭角色面板
  .render()               // 渲染内容
```

#### shop.js - 商店
```javascript
window.UIShop
  .init()                 // 初始化
  .open()                 // 打开商店
  .close()                // 关闭商店
  .refreshItems()          // 刷新商品
  .buyItem(index)         // 购买物品
  .sellEquipment()         // 出售装备
```

#### bestiary.js - 图鉴
```javascript
window.UIBestiary
  .init()                 // 初始化
  .open()                 // 打开图鉴
  .close()                // 关闭图鉴
  .setTab(tab)           // 切换标签页
  .render()               // 渲染内容
```

### 3.3 渲染模块

#### renderUtils.js - 统一渲染工具
```javascript
window.RenderUtils
  .getItemTooltip()              // 获取物品提示
  .getItemNameHtml()             // 获取物品名称HTML
  .getItemSlotHtml()             // 获取物品格子HTML
  .getEquipSlotHtml()            // 获取装备槽HTML
  .getShopItemHtml()             // 获取商店物品HTML
  .getBestiaryMonsterHtml()      // 获取图鉴怪物HTML
  .getBestiarySkillHtml()       // 获取图鉴技能HTML
  .getBestiaryEquipmentHtml()   // 获取图鉴装备HTML
  .compareItems()                // 对比装备属性
```

#### renderer.js - 游戏渲染
```javascript
drawMap()               // 绘制地图
drawPlayer()            // 绘制玩家
drawEnemies()           // 绘制敌人
drawBoss()              // 绘制Boss
drawDrops()            // 绘制掉落物
drawProjectiles()       // 绘制投射物
drawParticles()         // 绘制粒子
drawClouds()            // 绘制云朵
drawUI()                // 绘制UI
drawDamageNumbers()     // 绘制伤害数字
```

---

## 4. 核心机制

### 4.1 交互功能列表

#### 背包 (inventory.js)
- 点击物品 → 打开对比面板 (UICompare.show)
- 右键 → 出售该物品
- 点击装备槽 → 装备物品
- 整理按钮 → 合并相同物品（叠加）
- 快速出售 → 一键出售指定品质装备

#### 商店 (shop.js)
- 点击物品 → 打开对比面板（显示属性）
- 购买 → 扣金币，物品入背包，商店物品移除
- 刷新 → 消耗50金币刷新商品
- 出售装备 → 弹出确认框，一键出售背包中非装备物品

#### 对比面板 (compare.js)
- 点击装备 → 装备物品
- 点击购买 → 购买物品
- 关闭 → 返回原面板

#### 角色面板 (character.js)
- 显示装备、属性、等级
- 点击装备槽 → 打开对比面板

#### 图鉴 (bestiary.js)
- 标签页：怪物/Boss/技能/装备
- 显示发现状态和击杀统计

### 4.2 战斗系统

| 技能 | 类型 | MP消耗 | 冷却 | 效果 |
|------|------|--------|------|------|
| 斩击 | 单体 | 0 | 0s | 基础攻击 |
| 火球 | 投射 | 15 | 60f | 火球术 |
| 雷电 | 投射 | 20 | 80f | 雷电术 |
| 藤蔓 | 投射 | 15 | 70f | 藤蔓术 |
| 龙卷 | 投射 | 25 | 100f | 旋风 |
| 冰霜 | 投射 | 18 | 75f | 冰锥术 |

### 4.2 装备品质

| 品质 | 颜色 | 属性加成 |
|------|------|---------|
| 普通 | 白色 #fff | 100% |
| 优秀 | 绿色 #4f4 | 130% |
| 稀有 | 蓝色 #44f | 160% |
| 史诗 | 紫色 #a4f | 200% |
| 传说 | 金色 #fa4 | 250% |

### 4.3 地图瓦片

| 类型 | ID | 描述 |
|------|-----|------|
| 地面 | 0 | 默认地面 |
| 墙壁 | 1 | 障碍物（仅边界） |
| 草丛 | 2 | 装饰（4种样式） |
| 水域 | 3 | 障碍物 |
| 小山 | 4 | 装饰（4种样式） |
| 树木 | 5 | 障碍物/装饰（5种类型） |
| 花朵 | 6 | 装饰（6种颜色） |
| 石头堆 | 7 | 障碍物（3种样式） |

### 4.4 渲染字段

所有数据现在都包含 `render` 字段，用于统一渲染：
- 怪物: `render: 'slime'`, `'boss_dragon'` 等
- 装备: `sprite: 'fire_sword'`, `'dragon'` 等
- 技能: `render: 'fireball'`, `'ice'` 等

---

## 5. 控制方式

### 5.1 键盘控制
- W/↑: 上移
- S/↓: 下移
- A/←: 左移
- D/→: 右移
- 空格/J: 攻击
- 1-6: 使用技能

### 5.2 触屏控制
- 虚拟方向键（D-Pad）
- 攻击按钮

---

## 6. 测试

### 运行测试
在浏览器中打开：
- `tests/data-tests.html` - 数据模块测试
- `tests/ui-tests.html` - UI模块测试

---

## 7. 扩展指南

### 7.1 添加新物品
在 `js/data/items.js` 的 `BASE_ITEMS` 数组中添加，并实现 `renderEquipmentIcon` 中的渲染逻辑。

### 7.2 添加新技能
在 `js/data/skills.js` 的 `SKILLS` 数组中添加，并实现 `renderSkillIcon` 中的渲染逻辑。

### 7.3 添加新怪物
在 `js/data/enemies.js` 的 `ENEMY_TYPES` 或 `BOSS_TYPES` 中添加，并实现 `renderEnemyIcon` 中的渲染逻辑。

### 7.4 添加新渲染类型
在 `js/renderer.js` 的 `drawPixelSprite` 函数中添加新的渲染分支。

---

## 8. 许可证

本项目为开源项目，可自由学习和修改。

---

*文档版本: 1.22*  
*最后更新: 2026-02-18*

### 更新日志

**v1.22** (2026-02-18)
- **药水图标重新设计**:
  - 使用像素风格瓶身渲染
  - 玻璃质感瓶身、液体、气泡效果
  - 红药水(HP)和蓝药水(MP)不同颜色区分
  - 普通/超级药水视觉区分
- **角色装备动画**:
  - 护甲呼吸动画 + 传说/史诗品质发光脉动
  - 头盔上下浮动 + 高光闪烁
  - 靴子左右脚交替摆动
  - 戒指脉动缩放 + 品质发光
  - 项链左右摇摆 + 品质发光

**v1.21** (2026-02-18)
- **药水掉落渲染**:
  - 添加详细的治疗/魔法药水瓶图形
  - 液体动画效果和气泡粒子
  - 根据药水类型显示不同颜色（红色HP/蓝色MP）
  - 发光效果增强视觉效果
- **药水使用特效**:
  - 饮用时添加粒子爆炸效果
  - 上升的光粒子效果
  - 扩展的环形光效

**v1.20** (2026-02-18)
- **玩家攻击效果优化**:
  - 添加多层斩击轨迹和残影效果
  - 根据武器类型添加元素特效（火焰/雷电/冰霜）
  - 添加攻击终点火花粒子和闪光效果
  - 击中时添加屏幕震动效果
  - 武器挥动时添加发光效果
- **敌人攻击效果优化**:
  - 添加爪击、咬击、刺击三种攻击动画
  - 不同敌人类型拥有不同的攻击视觉效果
  - 攻击时显示红色爪痕、毒液飞溅、尾刺轨迹等
  - 添加攻击预示动画

**v1.19** (2026-02-18)
- **墙壁优化**: 移除墙壁动态阴影，保留底部边缘阴影保持立体感

**v1.18** (2026-02-18)
- **商店界面全新设计**:
  - 采用居中弹窗布局，更现代的游戏UI风格
  - 添加品质颜色边框（普通灰/优秀绿/稀有蓝/史诗紫/传说金）
  - 商品卡片悬停效果和点击反馈动画
  - 金币显示添加发光效果和金币图标
  - 刷新/出售按钮添加功能图标和悬停效果
  - 响应式布局适配移动端
  - 优化商品图标渲染，支持像素装备图标
  - 添加商品类型标签和品质标识
  - 优化空商店提示和错误提示

**v1.17** (2026-02-18)
- **墙壁优化**: 修复砖块超出边界的问题，砖块现在整齐排列在墙内
- **水域重新设计**: 
  - 添加渐变背景（深蓝到浅蓝）
  - 多层波纹动画（3种颜色波浪）
  - 波光粼粼的高光反射效果
  - 动态水花粒子
- **石头堆重构**:
  - 提取为独立元素类型 (TILE_TYPES.STONE = 7)
  - 移至 world.js 作为基础环境元素
  - 添加 STONE_TYPES 数组（堆叠、散落、巨石三种）
  - 创建 window.drawStonePile() 通用渲染函数

**v1.16** (2026-02-18)
- **怪物动画全面优化**（除蝙蝠外）:
  - 史莱姆: 弹跳果冻效果
  - 哥布林: 呼吸+耳朵抖动+武器摆动
  - 蜘蛛: 腹部脉动+8条腿蠕动
  - 骷髅: 呼吸+手臂摆动+发光眼睛+骨剑
  - 狼: 呼吸+尾巴摇摆+耳朵抖动+发光眼睛
  - 蛇: 波浪游动+舌头伸缩
  - 蝎子: 呼吸+钳子开合+尾巴卷曲
- **Boss动画增强**:
  - 史莱姆王: 弹跳+波浪边缘+皇冠抖动
  - 哥布林领主: 呼吸+角抖动+手臂摆动+巨斧
  - Orc王: 胸部扩张+肌肉起伏+战斧摆动
  - 黑暗法师: 漂浮+法杖发光脉动+符文旋转
  - 火龙: 翅膀扇动+龙息动画+尾巴摇摆
  - 冰魔: 水晶漂浮+粒子环绕
  - 恶魔领主: 翅膀扇动+火焰脉动+爪击+尾巴摆动

**v1.15** (2026-02-18)
- **动态阴影系统**: 所有元素根据时间动态变化阴影方向
- **重制小山**: 4种新样式（圆润、陡峭、缓坡、双峰），带自然地形效果
- **重制草丛**: 4种新样式（三叶草、茂密、芦苇、扇形），独立草叶动画
- **重制云朵**: 分层渲染+地面投影，增加立体感
- **新增石头堆类型**: 3种样式（堆叠石、散落石、巨石）
- **墙壁限制**: 墙壁只在地图边界生成，内部使用石头堆代替
- **统一阴影**: 地图元素、角色、怪物、Boss、武器全部使用动态阴影

**v1.14** (2026-02-18)
- 重新设计玩家角色像素艺术（原神风格）
- 添加悬浮武器系统，武器环绕角色做圆周运动
- 实现武器层级管理（前后遮挡效果）
- 武器始终垂直朝下，带发光效果和能量连线

**v1.13** (2026-02-18)
- 修复Boss渲染错误
- 优化所有图标渲染，统一返回canvas对象
- 完善drawImage错误处理机制

**v1.12** (2026-02-18)
- 重新设计所有怪物和Boss的像素艺术外观
- 添加动态动画效果（翅膀扇动、果冻弹跳等）
- 为怪物和Boss添加发光效果

**v1.11** (2026-02-18)
- 重新设计所有技能效果和图标
- 实现图标与游戏内效果一致
- 添加新的颜色属性系统

**v1.10** (2026-02-18)
- 重新设计所有装备的像素艺术
- 为每件装备添加独特设计（把手、刀镡、刃等）
- 优化玩家穿戴装备时的显示效果
